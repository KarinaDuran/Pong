...
public class Player {
	...
	public void moveUp(long delta) {
		// Verifica se o retangulo do player ta dentro dos limites
		if (cy - (height / 2) > v_limit[0]) {
			// Se tiver, calcula o novoCy e verifica se ele ultrapassa os limites
			double novoCy = cy - delta * speed;
			double diferenca = (novoCy - height / 2) - v_limit[0];
			if (diferenca < 0) {
				// Se sim, diminui o cy pra ficar do tamanho correto
				novoCy -= diferenca;
			}
			// Atualiza o cy
			this.cy = novoCy;
	...
	public void moveDown(long delta) {
		if (cy + (height / 2) < v_limit[1]) {
			double novoCy = cy + delta * speed;
			double diferenca = v_limit[1] - (novoCy + height / 2);
			if (diferenca < 0) {
				novoCy += diferenca;
			}
			this.cy = novoCy;
